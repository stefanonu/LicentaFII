...
public class BluetoothLeService extends Service {
    ...
    public final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        @Override
        public void onCharacteristicChanged(BluetoothGatt gamgattupdatett,
                                            BluetoothGattCharacteristic characteristic) {
            try {
                broadcastUpdate(ACTION_DATA_AVAILABLE, characteristic);
            } catch (UnsupportedEncodingException e) {
    ...
    private String  displayRxData(byte[] data) throws UnsupportedEncodingException {
        if (data != null) {
    ...
    private void broadcastUpdate(final String action,
                                 final BluetoothGattCharacteristic characteristic) throws UnsupportedEncodingException {
        final Intent intent = new Intent(action);

        //if (GattAttributes.IsUartTxCharacteristic(characteristic.getUuid()) >= 0) {
         //   Log.d(TAG, String.format("Received TX: %b",characteristic.getValue() ));
        ...
        if (data != null && data.length > 0) {
            ...
            Log.d("tag","value ddata => " + new String(data) + "\n" +
                    stringBuilder.toString() );
            }
        //}
        sendBroadcast(intent);
